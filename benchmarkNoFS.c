...
int main(int argc, char *argv[])
{
	...
	else if ((strcmp(argv[1], "-c") == 0) && argc == 5)
	{
		...
		// evito di usare FS e mi carico tutto in memoria direttamente
		// e mando tutto da li
		char *dataSample = (char *)malloc(transferSize);
		int rand_src = open("/dev/random", O_RDONLY);
		read(rand_src, dataSample, transferSize);
		close(rand_src);
